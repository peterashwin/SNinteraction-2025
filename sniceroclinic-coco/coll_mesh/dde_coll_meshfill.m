...
default={'acc',1,'doublecount',true,'scal',@(x)x,'grid','linear',...
    'purpose','storage','shapedim',1};
options=dde_set_options(default,varargin,'pass_on');
grid=dde_coll_set_grid(options.purpose,degree,'type',options.grid);
if ~options.doublecount && grid(1)==-1 && grid(end)==1
    append=tcoarse(end);
    grid=grid(1:end-1);
else
    append=[];
...
if options.doublecount && options.shapedim==2
    newmesh=reshape(newmesh,[],ndt);
end
end
