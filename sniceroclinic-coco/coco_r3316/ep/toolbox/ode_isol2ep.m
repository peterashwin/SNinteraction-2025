...
%ODE_ISOL2EP   Construct equilibrium point continuation problem from initial guess.
%
% PROB = ODE_ISOL2EP(PROB, OID, VARARGIN)
% VARARGIN = { F [DFDX [DFDP [DFDXDX [DFDXDP [DFDPDP]]]]] X0 [PNAMES] P0 [OPTS] }
% OPTS = { '-ep-end' | '-end-ep' | '-var' VECS }
%
% Construct continuation problem for dynamic equilibria of evolution
% equations of the form x'=f(x,p), or static equilibria of an equation of
...
%     {'ka' 'la'}, [-0.5 0.5]);
%
% will perform continuation with run name RUN='1' along a family of
% equilibria of x'=ka-x*(la-x^2) under variations of ka within the
% computational domain -0.5<=ka<=0.5, and return the computed bifurcation
% data as the cell array bd. Please see the demos shipped with EP for more
% examples of use. The meaning of the arguments EP_ARGS = { func, 0, {'ka'
% 'la'}, [0; 0.5] } and CONT_ARGS = { {'ka' 'la'}, [-0.5 0.5] } is
% explained below.
%
% On input:
%
...
%          with a prior call to COCO_SET, which will influence the behavior
%          of the EP toolbox. Execute EP_SETTINGS on the Matlab command
%          line to see an overview of EP toolbox settings.
%
% OID    : Object instance identifier (string). The corresponding toolbox
%          instance identifier is coco_get_id(OID, 'ep'). Pass the empty
%          string '' for a simple continuation of equilibria. Pass
...
%          options may be given). Either '-ep-end' or '-end-ep' mark the
%          end of input to ODE_ISOL2EP. The option '-var' indicates the
%          inclusion in the continuation problem of the variational problem
%          J*v=w, where the initial solution guess for v is given by the
%          content of VECS.
%
...
% EP toolbox settings must be assigned to prob before this call.
%
...
%   prob = coco_prob();
%   ...
%   prob = coco_set(prob, 'ep', EP_PROP, EP_PROP_VAL)
%   ...
...
% inclusion of EP-dependent user-defined monitor functions between the
% calls to ODE_ISOL2EP and COCO, as demonstrated in the demos.
%
% See also: COCO_SET, EP_SETTINGS, EP_READ_SOLUTION, EP_ADD, EP_ADD_VAR

% Copyright (C) Frank Schilder, Harry Dankowicz
% $Id: ode_isol2ep.m 3314 2024-09-02 15:31:13Z hdankowicz $
...
% vector field assumed to be autonomous
data.dfdthan   = [];
data.dfdtdxhan = [];
data.dfdtdphan = [];
data.dfdtdthan = [];
