classdef CurveSegment < CurveSegmentBase & EventLocator
  ...
  methods (Static) % utility functions
    ...
    function [prob, chart, TS] = nullspace(prob, chart)
...
%         [L,U,P,Q,R] = lu(J', [0.1 0.001]);
%         [m, n] = size(J);
%         Y  = L(1:m, 1:m)' \ L(m+1:end, 1:m)';
%         C1 = R\(P'*[Y; -speye(n-m)]);
        elseif d>n-m
          [~,~,C] = svds(J'*J, d, 'smallest');
          [B,~,~] = svds(J*J', d-n+m, 'smallest');
        else
          error('%s: incompatible manifold dimension', mfilename);
        end
      else
        if d==0
          [B,~,~] = svds(J*J', d-n+m, 'smallest');
...
%       C = zeros(numel(fidx), n);
%       B = zeros(m, numel(fidx)-n+m);
%       if n>m
%         [L, U, P]  = lu(J'); %#ok<ASGLU>
%         Y  = L(1:m, 1:m)' \ L(m+1:end, 1:m)';
%         C(1:n-m,:) = [Y; -speye(n-m)]'*P;
%         if numel(fidx)>n-m
%           [u,s,v] = svds([J; C(1:n-m,:)], numel(fidx)-n+m, 'smallest'); %#ok<ASGLU>
%           C(n-m+(1:numel(fidx)-n+m),:) = v';
%           B = u(1:m,:);
%         end
%       elseif n<m
%         [L, U, P]  = lu(J); %#ok<ASGLU>
...
%         if numel(fidx)>0
%           [u,s,v] = svds([J B(:,1:m-n)], numel(fidx), 'smallest'); %#ok<ASGLU>
%           C = v(1:n,:)';
%           B(:,m-n+(1:numel(fidx))) = u;
%         end
%       else
%         if numel(fidx)>0
%           [u,s,v] = svds(J, numel(fidx), 'smallest'); %#ok<ASGLU>
%           C = v(1:n,:)';
%           B = u(1:m,:);
%         end
%       end
...
%       TS = orth(full(TS));
  ...
  methods (Access=public) % interpolation functions
    ...
    function [opts, cseg, TS] = init_TS(cseg, opts, Valpha, h0, N, branch)
      ...
      if all(t0 == 0)
        ...
        if branch %find candidate direction for secondary branch
          fidx = coco_get_func_data(opts, 'cseg.prcond', 'fidx');
          [opts, cseg.curr_chart, J] = ...
            opts.efunc.DFDX(opts, cseg.curr_chart, cseg.curr_chart.x);
          J(fidx,:) = [];
          
          [m, n] = size(J);
          NV = zeros(m,1-n+m);
          if n<=m
            [Bm,~,~] = svds(J*J', 1-n+m, 'smallest');
            NV = orth(full(Bm));
          end
          
          [L,U,P,Q,R] = lu([J NV; chart.t' zeros(1,1-n+m)], ...
            opts.lsol.thresh);
          DR  = full(diag(R));
          DU  = full(diag(U));
...
%         end
    ...
    function [opts, chart] = update_TS(cseg, opts, chart)
      ...
      chart.TS          = cseg.orth(TS(1:nJ,:));
      dim  = mJ-nJ;
      if dim>0
        b                 = zeros(mJ, dim);
        b(nJ+1:end,:)     = eye(dim,dim);
        [opts, chart, NV] = opts.lsol.solve(opts, chart, J', b);
        chart.NV          = cseg.orth(NV(1:mJ-numel(fidx),:));
      end
    end
