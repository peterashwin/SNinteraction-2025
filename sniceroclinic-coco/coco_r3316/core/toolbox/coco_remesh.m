function [prob, chart, x0, V0] = coco_remesh(prob, chart, x0, V0, RMMX)
...
%% Ensure unchanged continuation parameters for embedded monitor functions

for i = efunc.embedded
  func  = efunc.funcs(i);
  f_idx = func.f_idx;
  for j=1:numel(func.pnames)
    prob = coco_set_parival(prob, func.pnames{j}, ...
      efunc.F_par_vals(f_idx(j)));
  end  
end
...
% change to accommodate overconstrained problems: max(ipend+1,1) instead of ipend+1
efunc.op_idx = [ cont_par_idx(1:ppnum) ...
  tmp.internal_pars(max(ipend+1,1):end) cont_par_idx(ppnum+1:end) ];
