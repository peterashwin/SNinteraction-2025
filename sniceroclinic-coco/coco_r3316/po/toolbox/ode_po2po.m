...
% OPTS = { '-switch' | '-po-end' | '-end-po' | '-var' VECS | '-no-var' }
%
...
%        ODE_PO2PO to switch branches at the solution point, which must
%        then be a branch point. Either '-po-end' or '-end-po' mark the end
%        of input to ODE_PO2PO. The option '-var' indicates the inclusion
...
%        VECS. Alternatively, the option '-no-var' indicates the exclusion
%        of the variational problem.
%
% See also: ODE_ISOL2PO, PO_READ_SOLUTION, PO_ADD

% Copyright (C) Frank Schilder, Harry Dankowicz
% $Id: ode_po2po.m 3313 2024-06-26 13:35:21Z hdankowicz $
...
if  ~opts.novar
  ...
  if opts.switch
    prob = ode_BP2coll(prob, tsid, args.run, ssid, args.lab, '-no-var');
  else
